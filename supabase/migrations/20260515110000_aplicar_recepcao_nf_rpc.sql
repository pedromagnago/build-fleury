-- ============================================================================
-- aplicar_recepcao_nf — RPC atômica que SUBSTITUI o aplicar() client-side
--
-- Motivo: o aplicar() em src/pages/RecepcaoPage.tsx fazia ~12 operações
-- sequenciais sem envelope transacional. Se qualquer passo falhasse no meio
-- (ex: insert de recepcao_consumos), recepcao_docs já estava criado e
-- pedido_itens.qtd_recebida já estava incrementado — deixando "NF meio
-- aplicada" sem como reverter. Esta RPC envelopa tudo numa única transação:
-- erro em qualquer passo = rollback completo.
--
-- Resolve também o bug "consumo fantasma ao reaplicar NF":
--   - Detecta duplicidade por chave_acesso (UNIQUE da migration anterior)
--   - Se p_force_replace_doc_id = id existente → deleta o doc antigo
--     (trigger BEFORE DELETE devolve qtd_recebida + restaura snapshots),
--     depois aplica a nova versão.
--
-- Simplificação consciente: a regeneração de "parcelas de saldo" nos pedidos
-- consumidos parcialmente gera UMA parcela única (em vez de portar o
-- gerarParcelas TS para PL/pgSQL). O operador é avisado via warning no return.
-- ============================================================================
...
DECLARE
  ...
  -- Snapshot acumulado (vai pro recepcao_consumos do âncora)
...
BEGIN
  ...
  -- ==========================================================================
  -- 2) DEDUP por chave_acesso (defense-in-depth — o front também pré-checa)
  -- ==========================================================================
  IF length(COALESCE(v_chave_acesso,'')) = 44 THEN
    ...
    IF v_existing_doc_id IS NOT NULL THEN
      ...
      END IF;
      -- Substituição autorizada: deleta o doc antigo. O trigger BEFORE DELETE
      -- faz toda a reversão (devolve qtd_recebida, restaura parcelas, apaga âncora).
  ...
  -- ==========================================================================
  -- 5) MONTAGEM das linhas elegíveis (acao ∈ substituir/criar_pedido + item_compra_id)
  -- ==========================================================================
  ...
  -- Conta linhas ignoradas (não-elegíveis)
  ...
  -- ==========================================================================
  -- 6) CONSUMO FIFO em pedido_itens (linhas com acao='substituir_pedido')
  -- ==========================================================================
  ...
  LOOP
    ...
    -- Sobra → marca como 'sobra_p_ancora'
  ...
  -- ==========================================================================
  -- 8) PARCELAS — apaga pendentes, regera saldo (1 parcela), insere parcelas do header
  -- ==========================================================================
  IF v_novo_pedido_id IS NOT NULL AND array_length(v_pedidos_consumidos_ids, 1) > 0 THEN
    ...
    -- 8.2) Regenera saldo (UMA parcela por pedido consumido — simplificação)
    FOR rec_ped IN
      SELECT pi.pedido_id,
             pi.valor_total_real,
             pi.cond_pagamento,
    ...
    LOOP
      ...
      IF v_saldo > 0.01 THEN
        ...
        INSERT INTO parcelas (
          id, company_id, pedido_id, numero_parcela,
          valor, data_vencimento, status, descricao, tipo
        ) VALUES (
          v_parcela_saldo_id, v_company_id, rec_ped.pedido_id,
  ...
  -- 8.3) Parcelas do âncora (vindas do header do operador)
  IF v_novo_pedido_id IS NOT NULL AND v_parcelas_count_payload > 0 THEN
    INSERT INTO parcelas (
      company_id, pedido_id, numero_parcela,
  ...
  -- ==========================================================================
  -- 9) LOG recepcao_consumos (deltas em planejados + linha do âncora)
  -- ==========================================================================
  ...
  -- ==========================================================================
  -- 10) LOG recepcao_matches (auditoria — todas as linhas do payload)
  -- ==========================================================================
...
COMMENT ON FUNCTION public.aplicar_recepcao_nf(jsonb) IS
  'RPC atômica que substitui o aplicar() client-side da RecepcaoPage. '
  'Resolve: (1) atomicidade (transação única — erro = rollback completo), '
  '(2) detecção de NF duplicada por chave_acesso (com force_replace opcional), '
  '(3) consistência (recepcao_docs, pedidos, pedido_itens, parcelas, '
  'recepcao_consumos, recepcao_matches sempre coerentes ou tudo desfeito).';

GRANT EXECUTE ON FUNCTION public.aplicar_recepcao_nf(jsonb) TO authenticated;

-- ============================================================================
-- Função helper: front consulta antes de aplicar pra mostrar diálogo
-- "esta NF já foi aplicada — excluir e refazer?"
-- ============================================================================
CREATE OR REPLACE FUNCTION public.recepcao_doc_por_chave(p_company_id uuid, p_chave_acesso text)
RETURNS TABLE (
  id uuid, numero_doc text, fornecedor_nome text, valor_total numeric,
  data_emissao date, applied_at timestamptz
)
LANGUAGE sql
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$
  SELECT id, numero_doc, fornecedor_nome, valor_total, data_emissao, applied_at
   ...
   WHERE company_id = p_company_id
     AND chave_acesso = regexp_replace(COALESCE(p_chave_acesso,''), '\D','','g')
     AND public.user_can_access_company(auth.uid(), company_id)
   LIMIT 1;
$$;

GRANT EXECUTE ON FUNCTION public.recepcao_doc_por_chave(uuid, text) TO authenticated;
