-- Fix: aplicar_recepcao_nf não preenchia nf_origem_id nas parcelas criadas.
-- CentralNotas busca parcelas via nf_origem_id IN (docIds), então aparecia
-- "sem parcelas" mesmo quando parcelas existiam vinculadas ao pedido âncora.
--
-- Correções:
--   1) INSERT payload parcelas: adiciona nf_origem_id = v_doc_id + tipo = 'contratual'
--   2) INSERT parcela de saldo remanescente: adiciona nf_origem_id = v_doc_id
--
-- Backfill executado diretamente:
--   UPDATE parcelas SET nf_origem_id = p.nf_origem_id
--   FROM pedidos p
--   WHERE parcelas.pedido_id = p.id
--     AND parcelas.nf_origem_id IS NULL
--     AND p.nf_origem_id IS NOT NULL;
--   → 5 parcelas corrigidas
