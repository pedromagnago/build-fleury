-- Pedido como Previsão de Orçamento (opt-in)
--
-- Cenário: cliente cria pedido genérico com qtd=1 e valor alto pra registrar
-- placeholder de gasto no fluxo de caixa (sem rastrear unidade física). NFs reais
-- do MESMO ITEM (qualquer fornecedor) devem abater do SALDO FINANCEIRO da previsão,
-- sem consumir a quantidade fictícia. Veja conversação em 2026-05-18.
--
-- Default: is_previsao_orcamento=false (comportamento atual preservado).
-- Quando true:
--   • RPC aplicar_recepcao_nf consome o pedido por VALOR (incrementa
--     pedidos.valor_coberto_por_realizacao) em vez de por QUANTIDADE.
--   • qtd_recebida NÃO é tocada (qtd é fictícia, não representa entrega).
--   • View v_pedidos_fantasma_candidatos ignora previsões (não são fantasma).
--   • Recepção: F2.a oferece banner pra vincular NF a previsão com saldo.
--   • Excedente: se NF > saldo da previsão, sobra vira pedido novo (como hoje).
--
-- Saldo efetivo de uma previsão:
--   valor_total_real - SUM(parcelas.valor_pago) - valor_coberto_por_realizacao
--
-- recepcao_consumos.valor_coberto_previsao registra quanto cada NF cobriu de
-- cada previsão (NULL = consumo físico normal).
...
-- ============================================================================
ALTER TABLE pedidos
  ADD COLUMN IF NOT EXISTS is_previsao_orcamento boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS valor_coberto_por_realizacao numeric(15,2) NOT NULL DEFAULT 0
    CHECK (valor_coberto_por_realizacao >= 0);

COMMENT ON COLUMN pedidos.is_previsao_orcamento IS
  'true = pedido placeholder de previsão por item (qtd fictícia). NFs do mesmo item '
  'consomem por VALOR (não por qtd). Default false = pedido normal.';

COMMENT ON COLUMN pedidos.valor_coberto_por_realizacao IS
  'Soma das NFs externas que realizaram parte deste pedido previsão. Reduz o '
  'saldo efetivo a pagar sem tocar em parcelas.';

-- ============================================================================
-- 2) Coluna nova em recepcao_consumos pra rastrear cobertura
-- ============================================================================
ALTER TABLE recepcao_consumos
  ADD COLUMN IF NOT EXISTS valor_coberto_previsao numeric(15,2);

COMMENT ON COLUMN recepcao_consumos.valor_coberto_previsao IS
  'Quanto a NF cobriu desta previsão. NULL = consumo físico normal (use delta_qtd_recebida).';

-- ============================================================================
-- 3) RPC aplicar_recepcao_nf — suporte a previsão
-- ============================================================================
-- Adiciona um bloco no FIFO que detecta is_previsao_orcamento=true no pedido
-- substituído e age por VALOR. Quando consome um pedido normal, comportamento
-- preserva 100% o anterior.
...
BEGIN
  ...
  IF length(COALESCE(v_chave_acesso,'')) = 44 THEN
    ...
    IF v_existing_doc_id IS NOT NULL THEN
      IF v_force_replace_doc_id IS NULL OR v_existing_doc_id <> v_force_replace_doc_id THEN
        RAISE EXCEPTION 'NF com chave_acesso % já aplicada (doc_id %)', v_chave_acesso, v_existing_doc_id
  ...
  LOOP
    ...
    -- Se o operador especificou pedido_substituido_id, checa se é previsão
    v_is_previsao := false;
    ...
    IF v_is_previsao AND v_pedido_previsao_id IS NOT NULL THEN
      ...
      IF v_valor_cobrir > 0.01 THEN
        ...
        -- Quantidade restante segue proporcional ao valor
    ...
    -- ───────────────────────────────────────────────────────────────────────
    -- CONSUMO POR QUANTIDADE (pedidos normais — FIFO entre todos os planejados)
    -- Pula pedidos previsão (já tratados acima).
    -- ───────────────────────────────────────────────────────────────────────
    FOR rec_pi IN
  ...
  -- Pedido âncora (apenas pra sobras de consumo físico OU criar_pedido OU frete/parcelas sem consumo físico)
  ...
  IF v_precisa_ancora AND v_item_ancora_id IS NOT NULL THEN
    ...
    INSERT INTO pedidos (
      ...
      CASE WHEN v_pedido_ancora_sem_itens
        ...
        ELSE format('NF %s · %s', COALESCE(v_numero_doc,''), COALESCE(v_fornecedor_nome,'fornecedor sem nome'))
      END,
      v_doc_id
  ...
  -- Parcelas dos pedidos físicos consumidos
  IF v_novo_pedido_id IS NOT NULL AND array_length(v_pedidos_consumidos_ids, 1) > 0 THEN
    ...
    LOOP
      ...
      IF v_saldo > 0.01 THEN
        ...
        INSERT INTO parcelas (id, company_id, pedido_id, numero_parcela, valor,
          data_vencimento, status, descricao, tipo
        ) VALUES (
          ...
          'futura', format('Saldo após consumo NF %s', COALESCE(v_numero_doc,'?')), 'contratual'
        );
  ...
  IF v_novo_pedido_id IS NOT NULL AND v_parcelas_count_payload > 0 THEN
    INSERT INTO parcelas (
      company_id, pedido_id, numero_parcela, valor, data_vencimento, status, descricao
    )
  ...
  IF v_parcelas_preservadas_count > 0 THEN
    v_warnings := array_append(v_warnings,
      format('%s parcela(s) preservada(s) (baixa/conciliação). Revise em Compras.',
             v_parcelas_preservadas_count));
  END IF;
  IF array_length(v_parcelas_regeradas_ids, 1) > 0 THEN
    v_warnings := array_append(v_warnings,
      format('Saldo dos %s pedido(s) cobertos por 1 parcela única — redistribua se a condição era a prazo.',
             array_length(v_parcelas_regeradas_ids, 1)));
  END IF;
  IF v_previsoes_cobertas_count > 0 THEN
    v_warnings := array_append(v_warnings,
      format('%s previsão(ões) de orçamento cobertas por valor (saldo financeiro reduzido).',
             v_previsoes_cobertas_count));
  END IF;
...
ped_via_nf AS (
  SELECT p.company_id, p.id AS pedido_id, p.numero_pedido, p.fornecedor_id,
    f.nome AS fornecedor_nome, pi.item_compra_id, pi.qtd, pi.qtd_recebida,
    pi.valor_total_real, p.created_at, p.nf_origem_id
  FROM pedidos p
  JOIN pedido_itens pi ON pi.pedido_id = p.id
  ...
  WHERE p.nf_origem_id IS NOT NULL
    ...
    AND pi.qtd <= pi.qtd_recebida + 0.001
    AND NOT EXISTS (
      SELECT 1 FROM pedido_itens pi2 WHERE pi2.pedido_id = p.id
        AND pi2.qtd > pi2.qtd_recebida + 0.001
    )
    AND (p.observacoes IS NULL OR (
      p.observacoes NOT LIKE '%[absorvido%' AND p.observacoes NOT LIKE '%[recebeu absorcao%'
    ))
)
SELECT pnf.company_id, pnf.pedido_id AS fantasma_id, pnf.numero_pedido AS fantasma_numero,
  pnf.fornecedor_nome AS fornecedor_nf, pnf.item_compra_id, ic.codigo AS item_codigo,
  ic.descricao AS item_descricao, pnf.qtd AS qtd_fantasma,
  pnf.qtd_recebida AS qtd_recebida_fantasma, pnf.valor_total_real AS valor_fantasma,
  pnf.nf_origem_id, ppl.planejados_ids, ppl.planejados_numeros,
  ppl.saldo_qtd_total AS saldo_planejados_qtd, ppl.saldo_valor_total AS saldo_planejados_valor,
  pnf.created_at AS fantasma_criado_em
FROM ped_via_nf pnf
JOIN ped_planejados_nao_nf ppl
  ON ppl.company_id = pnf.company_id AND ppl.item_compra_id = pnf.item_compra_id
JOIN itens_compra ic ON ic.id = pnf.item_compra_id;

-- ============================================================================
-- 5) Trigger revert ao excluir recepcao_doc — reverte coberturas de previsão
-- ============================================================================
-- A trigger BEFORE DELETE existente reverte qtd_recebida. Precisamos também
-- reverter valor_coberto_por_realizacao das previsões cobertas por esse doc.
CREATE OR REPLACE FUNCTION public.fn_recepcao_doc_revert_consumo_previsao()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  FOR rec IN
    SELECT rc.pedido_item_id, rc.valor_coberto_previsao, pi.pedido_id
    FROM recepcao_consumos rc
    LEFT JOIN pedido_itens pi ON pi.id = rc.pedido_item_id
    WHERE rc.doc_id = OLD.id
      AND rc.valor_coberto_previsao IS NOT NULL
      AND rc.valor_coberto_previsao > 0
  LOOP
    IF rec.pedido_id IS NOT NULL THEN
      UPDATE pedidos
      SET valor_coberto_por_realizacao = GREATEST(
        COALESCE(valor_coberto_por_realizacao, 0) - rec.valor_coberto_previsao, 0
      )
      WHERE id = rec.pedido_id;
    END IF;
  END LOOP;
  RETURN OLD;
END;
...
DROP TRIGGER IF EXISTS trg_recepcao_doc_revert_previsao ON recepcao_docs;
CREATE TRIGGER trg_recepcao_doc_revert_previsao
BEFORE DELETE ON recepcao_docs
FOR EACH ROW
EXECUTE FUNCTION public.fn_recepcao_doc_revert_consumo_previsao();
